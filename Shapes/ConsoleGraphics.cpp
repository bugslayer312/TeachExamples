...
ConsoleGraphics::ConsoleGraphics(size_t width, size_t height) : m_width(width), m_height(height)
{
...
void ConsoleGraphics::SetPixel(int x, int y)
{
    m_data[(y - 1) * m_width + x - 1] = '#';
}

void ConsoleGraphics::DrawLine(int x1, int y1, int x2, int y2)
{
    if (y1 == y2)
    {
        if (y1 > 0 && y1 <= m_height)
...
void ConsoleGraphics::DrawRectangle(int x, int y, int width, int height)
{
    int x2 = x + width;
    int y2 = y + height;
...
void ConsoleGraphics::DrawEllipse(int x, int y, int width, int height)
{
